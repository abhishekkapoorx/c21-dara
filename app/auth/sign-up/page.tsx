...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: any) => {
    setIsLoading(true);
    try {
      const resIsDisp = await fetch(`https://open.kickbox.com/v1/disposable/${formData.email}`);
      const { disposable } = await resIsDisp.json();
      if (disposable) {
        setGotResponse("Please use a valid email address")
        return;
      }
